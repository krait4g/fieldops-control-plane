# Local Observe 실행 소스 Snapshot

이 저장소는 localhost에서 Synthetic 장비 데이터를 관찰하고 Synthetic Camera를 미리보기·제어하는 포트폴리오 Snapshot을 포함합니다. Production Release가 아니며, 실제 고객 데이터나 운영 Credential을 사용하지 않습니다. 이 문서를 포함한 Git commit이 공개 Snapshot의 정확한 소스 식별자입니다.
...
Billing, TCP/Binary, durable Command/Alarm, AI, Rule Engine 구현은 이 Snapshot에 포함되지 않습니다. Camera/PTZ는 한 대의 Synthetic 장비와 localhost media 경로만 포함하며 실제 Vendor 전체 호환이나 preset을 뜻하지 않습니다. 성능 benchmark, 고가용성, Production 배포, Tag, GitHub Release, Hosting도 포함하지 않습니다.
...
실행 방법은 [Local Observe Quick Start](LOCAL_OBSERVE_QUICKSTART.md)와 [Camera/PTZ Quick Start](CAMERA_PTZ_QUICKSTART.md)를 따릅니다.
